...
public class TextDisplayMeta extends AbstractDisplayMeta {
     ...
     * @param opacity The opacity of the background. 0 is transparent, 1 is opaque.
     */
    public void setBackgroundColor(@NotNull TextColor color, final float opacity) {
        final var java_color = Color.decode(color.asHexString());
        super.metadata.setIndex(OFFSET + 2, Metadata.VarInt(new Color(java_color.getRed(), java_color.getGreen(), java_color.getBlue(), opacity).getRGB()));
    }
